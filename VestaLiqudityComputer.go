...
`
    ...
    var (
	Daniel = p.NFS("3316.585777712171076053")
	Byta   = p.NFS("24987.628097349280102102")
    )

    Dani2 := sm.TruncateCustom(sm.DIVxc(Daniel, p.NFS("2")), 18)
    Rest := sm.SUBxc(Daniel, Dani2)
    Byta2 := sm.ADDxc(Byta, Rest)
    fmt.Println("Dani is", Dani2)
    fmt.Println("Byta is", Byta2)
}
